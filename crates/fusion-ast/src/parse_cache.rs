...
fn key_for(code: &str, lang: SupportLang) -> Key {
    Key {
        hash: xxhash_rust::xxh64::xxh64(code.as_bytes(), HASH_SEED),
        len: code.len(),
        lang,
    }
}
...
mod tests {
    ...
    fn language_is_part_of_the_key() {
        ...
        assert_ne!(
            py_key, ts_key,
            "same bytes, different language, different key"
        );
    ...
    fn verified_equality_rejects_a_colliding_slot() {
        ...
        assert_eq!(
            stored.len(),
            probe.len(),
            "collision needs a matching len field"
        );
    ...
    fn entry_bound_evicts_least_recently_used() {
        ...
        assert!(
            cache.get(&keys[0], sources[0]).is_some(),
            "recently used slot survives"
        );
        assert!(
            cache.get(&keys[1], sources[1]).is_none(),
            "least recently used slot evicted"
        );
        assert!(
            cache.get(&keys[2], sources[2]).is_some(),
            "newest slot resident"
        );
    }
    ...
    fn source_over_the_entry_bound_is_never_retained() {
        ...
        assert_eq!(
            cache.entries.len(),
            0,
            "oversized source is parsed but not cached"
        );
        assert_eq!(cache.source_bytes, 0);
        assert_eq!(
            cache.evictions, 0,
            "rejection must not evict a healthy slot"
        );
