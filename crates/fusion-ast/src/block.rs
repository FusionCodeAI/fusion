...
mod tests {
    ...
    fn repeat_query_yields_identical_boundaries() {
        ...
        assert_eq!(
            second, first,
            "identical input must yield identical boundaries"
        );
    ...
    fn syntax_error_stays_none_across_repeat_calls() {
        ...
        assert_eq!(
            boundaries(code, "b.ts", &[(1, 2)]),
            None,
            "different window, still none"
        );
    }
    ...
    fn cached_error_tree_matches_uncached_verdicts_for_both_callers() {
        ...
        assert_eq!(
            cold_boundaries, None,
            "a file-level error disables boundaries"
        );

        clear_parse_cache();
        for pass in 0..2 {
            assert_eq!(
                boundaries(code, "x.ts", &[(1, 1)]),
                cold_boundaries,
                "pass {pass}"
            );
    ...
    fn keys_evicted_past_the_cache_bound_still_answer_correctly() {
        ...
        assert_eq!(
            boundaries(&sources[last], "x.ts", &[(1, 1)]),
            expect(last),
            "resident key"
        );
    }
    ...
    fn pruned_walk_matches_unpruned_on_repo_corpus_sample() {
        ...
        if !root.join("packages").is_dir() {
            eprintln!(
                "skipping: repository corpus unavailable at {}",
                root.display()
            );
            return;
        }
        ...
        if files.is_empty() {
            eprintln!(
                "skipping: repository corpus scan found no sources at {}",
                root.display()
            );
            return;
        }
        assert!(
            files.len() > 40,
            "corpus sample too small to be evidence: {}",
            files.len()
        );
        let (comparisons, _) = sweep_corpus(&files);
        assert!(
            comparisons > 300,
            "expected a broad sweep, got {comparisons} comparisons"
        );
    }
    ...
    fn pruned_walk_matches_unpruned_on_full_repo_corpus() {
        ...
        let (comparisons, _) = sweep_corpus(&files);
        println!(
            "full corpus: {} files, {comparisons} comparisons",
            files.len()
        );
    }
    ...
    fn pruned_walk_matches_unpruned_on_error_and_degenerate_inputs() {
        ...
        assert!(
            none_verdicts > 0,
            "error / unknown-language cases must exercise the None path"
        );
    }
    ...
    fn window_deep_inside_nesting_still_returns_the_whole_chain() {
        ...
        assert_eq!(
            boundaries(&code, "nested.ts", &[(nest + 5, nest + 5)]),
            Some(vec![])
        );
    ...
    fn content_end_line_before_raw_end_is_still_surfaced() {
        ...
        assert_eq!(
            boundaries(code, "x.py", &[(4, 4)]),
            boundaries_unpruned(code, "x.py", &[(4, 4)])
        );

        // And the raw end line (the blank one) on its own.
        assert_eq!(
            boundaries(code, "x.py", &[(5, 5)]),
            boundaries_unpruned(code, "x.py", &[(5, 5)])
        );
