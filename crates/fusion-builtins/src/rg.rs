...
struct RgWalk {
	request: fusion_walker::WalkRequest,
	filters: PathFilters,
}
...
impl PathFilters {
	fn includes(&self, path: &Path, file_type: fusion_walker::FileType, size: Option<f64>) -> bool {
		let is_dir = file_type == fusion_walker::FileType::Dir;
		...
		if file_type != fusion_walker::FileType::File {
			return true;
		}
		if !explicitly_included
			&& self
...
fn build_walk(host: &mut Host, cli: &Rg, root: &Path) -> Result<RgWalk, String> {
	...
	let order = if cli.sort_files || cli.sort.as_deref() == Some("path") {
		fusion_walker::WalkOrder::Path
	} else {
		fusion_walker::WalkOrder::Unordered
	};
	let request = fusion_walker::WalkRequest::new(root)
		.hidden(include_hidden)
		.gitignore(!no_ignore)
		.skip_git(!no_ignore)
		.skip_node_modules(false)
		.follow_links(fusion_walker::FollowLinks::from(cli.follow && !cli.no_follow))
		.detail(if filters.max_filesize.is_some() {
			fusion_walker::WalkDetail::Full
		} else {
			fusion_walker::WalkDetail::Minimal
		})
		.order(order)
		.emit_root(false)
	...
	let streamed = match walk.request.for_each_entry_with_heartbeat(
		...
		|entry| {
			...
			if !walk.filters.includes(path, entry.file_type, entry.size) {
				return Ok(if entry.file_type == fusion_walker::FileType::Dir {
			...
			Ok(if opts.quiet && any_match.get() {
				fusion_walker::WalkDecision::Stop
			} else {
				fusion_walker::WalkDecision::Include
			})
		},
		|error| {
			had_error.set(true);
...
fn collect_filtered_files(host: &mut Host, cli: &Rg, root: &Path) -> io::Result<Vec<PathBuf>> {
	...
	let result = walk.request.for_each_entry_with_heartbeat(
		...
		|_| Ok(fusion_walker::WalkDecision::Include),
	);
	match result {
		Ok(fusion_walker::WalkStatus::Complete | fusion_walker::WalkStatus::Stopped) => Ok(files),
		Err(fusion_walker::WalkError::Interrupted(_)) if host.is_cancelled() => {
