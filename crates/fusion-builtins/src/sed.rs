...
pub mod compiler {
            ...
            Some(
                crate::bre::bre_to_ere(pattern, crate::bre::Backrefs::Supported).map_err(|e| {
                    compilation_error::<Regex>(
                        lines,
                        line,
                        format!("invalid regex '{pattern}': {}", e.message()),
                    )
                    .unwrap_err()
                })?,
            )
    ...
    mod tests {
        ...
        fn test_compile_re_basic_leading_plus_is_a_literal() {
            ...
            assert!(
                regex
                    .is_match(&mut IOChunk::new_from_str("+added"))
                    .unwrap()
            );
            assert!(!regex.is_match(&mut IOChunk::new_from_str("alpha")).unwrap());
            assert!(
                !regex
                    .is_match(&mut IOChunk::new_from_str(" context"))
                    .unwrap()
            );
        }
        ...
        fn test_compile_re_basic_no_operand_brace_is_rejected() {
            ...
            for pattern in [r"\{1,3\}", r"^\{1,3\}"] {
                ...
                assert!(
                    err.to_string()
                        .contains("repetition-operator operand invalid"),
                    "{pattern}: {err}"
                );
            }
        }
        ...
        fn test_compile_re_basic_caret_is_an_anchor_inside_a_group() {
            ...
            assert!(
                !regex
                    .is_match(&mut IOChunk::new_from_str("xalpha"))
                    .unwrap()
            );
        }
...
mod tests {
    ...
    fn builtin_substitutes_only_plus_prefixed_lines() {
        ...
        let input = "alpha\n+added\n-removed\n context\n+another\n";
        let (code, capture) = crate::host::run_util::<Sed>(&[r"s/^\+/PLUS/"], input, "/");
