...
use fusion_walker::CollectedEntry;
...
fn search(
	...
	for search_path in &search_paths {
		...
		let outcome = match request.collect_with_heartbeat(cancel_heartbeat(cancelled)) {
			Ok(outcome) => outcome,
			Err(fusion_walker::WalkError::Interrupted(_)) if cancelled.load(Ordering::Relaxed) => break,
...
fn fd_walk_request(
	...
	fusion_walker::WalkRequest::new(root)
		.hidden(include_hidden(cli))
		.gitignore(use_gitignore)
		.skip_git(false)
		.skip_node_modules(false)
		.follow_links(cli.follow.into())
		.detail(fusion_walker::WalkDetail::Minimal)
		.order(fusion_walker::WalkOrder::Path)
		.emit_root(true)
		.depth(min_depth, max_depth)
		.directory_errors(fusion_walker::DirectoryErrorMode::Visit)
		.same_file_system(same_file_system)
		.cache(false)
		.visit_order(fusion_walker::VisitOrder::PreOrder)
}
		...
		let status = request.for_each_entry_with_heartbeat(
			...
			|entry| {
				if cancelled.load(Ordering::Relaxed) || max_results.is_some_and(|max| matches >= max) {
					return Ok(fusion_walker::WalkDecision::Stop);
				}
				...
				if cancelled.load(Ordering::Relaxed) || max_results.is_some_and(|max| matches >= max) {
					Ok(fusion_walker::WalkDecision::Stop)
				} else {
					Ok(decision)
				}
			},
			|error| {
				...
				Ok(fusion_walker::WalkDecision::Include)
			},
		);
		state.matches = matches;
		state.had_error = had_error;
		match status {
			Ok(fusion_walker::WalkStatus::Complete | fusion_walker::WalkStatus::Stopped) => {},
			Err(fusion_walker::WalkError::Interrupted(_)) if cancelled.load(Ordering::Relaxed) => break,
...
fn process_walker_entry<W: Write>(
	...
	path: &Path,
	depth: usize,
	file_type: fusion_walker::FileType,
	out: &mut W,
	matches: &mut usize,
) -> io::Result<fusion_walker::WalkDecision> {
	let is_directory = file_type == fusion_walker::FileType::Dir;
	if depth == 0 && is_directory {
		return Ok(fusion_walker::WalkDecision::Skip);
	}
	if config.excludes.matches(path, &config.base_dir) {
		return Ok(if is_directory {
			fusion_walker::WalkDecision::SkipDescend
		} else {
			fusion_walker::WalkDecision::Skip
		});
	}
	if is_directory {
		if ignore_contains.iter().any(|name| path.join(name).exists()) {
			return Ok(fusion_walker::WalkDecision::SkipDescend);
		}
		if config.prune
			&& config
				.matcher
	...
	*matches = (*matches).saturating_add(1);
	if config.quiet {
		return Ok(fusion_walker::WalkDecision::Include);
	}
...
fn matches_walker_type_filter(
	...
	let kind_matches = if filter.has_kind() {
		(filter.regular && file_type == fusion_walker::FileType::File)
			|| (filter.directory && file_type == fusion_walker::FileType::Dir)
			|| (filter.symlink && file_type == fusion_walker::FileType::Symlink)
	} else {
		true
	};
	if !kind_matches {
...
/// [`fusion_walker::WalkError::Interrupted`], which the callers translate to a
...
fn walker_error_to_io(err: fusion_walker::WalkError<io::Error>) -> io::Error {
	match err {
		fusion_walker::WalkError::Interrupted(err) => err,
		fusion_walker::WalkError::InvalidData { path, message } => {
			io::Error::other(format!("{}: {message}", path.display()))
		},
	}
}

fn walker_collect_error_to_io(err: fusion_walker::WalkError<String>) -> io::Error {
	match err {
		fusion_walker::WalkError::Interrupted(err) => io::Error::other(err),
		fusion_walker::WalkError::InvalidData { path, message } => {
			io::Error::other(format!("{}: {message}", path.display()))
		},
	}
...
fn process_collected_entry<W: Write>(
	...
	let is_directory = entry.file_type == fusion_walker::FileType::Dir;
	if depth == 0 && is_directory {
		return Ok(());
	}
...
mod tests {
	...
	fn walk_request(tree: &std::path::Path) -> fusion_walker::WalkRequest {
		fusion_walker::WalkRequest::new(tree)
			.hidden(true)
			.gitignore(false)
			.emit_root(true)
	}
	...
	fn cancel_heartbeat_aborts_collect_with_heartbeat() {
		...
		assert!(
			matches!(err, fusion_walker::WalkError::Interrupted(_)),
	...
	fn cancel_heartbeat_aborts_for_each_entry_with_heartbeat() {
		...
		let result = walk_request(tree.path()).for_each_entry_with_heartbeat(
			...
			|_entry| {
				...
				Ok::<_, std::io::Error>(fusion_walker::WalkDecision::Include)
			},
			|_error| Ok::<_, std::io::Error>(fusion_walker::WalkDecision::Include),
		);
