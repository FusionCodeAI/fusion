...
fn walker_follow_links(follow: Follow) -> fusion_walker::FollowLinks {
    match follow {
        Follow::Never => fusion_walker::FollowLinks::Never,
        Follow::Roots => fusion_walker::FollowLinks::Roots,
        Follow::Always => fusion_walker::FollowLinks::Always,
    }
}

fn build_find_walk_request(config: &Config, root: &Path) -> fusion_walker::WalkRequest {
    fusion_walker::WalkRequest::new(root)
        ...
        .visit_order(if config.depth_first {
            fusion_walker::VisitOrder::ContentsFirst
        } else {
            fusion_walker::VisitOrder::PreOrder
        })
...
fn process_dir_walk_request(
    ...
    let status = request.for_each_entry_with_heartbeat(
        ...
        |entry: fusion_walker::EntryMeta<'_>| {
            ...
            if host.sigpipe_hit() {
                ...
                Ok(fusion_walker::WalkDecision::SkipDescend)
            } else {
    ...
    match status {
        ...
        Err(fusion_walker::WalkError::Interrupted(err)) => {
            ret_value = 1;
