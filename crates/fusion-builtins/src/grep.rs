...
fn resolve_follow_links(cli: &GrepArgs, matches: &ArgMatches) -> fusion_walker::FollowLinks {
	let mut selected = (0, fusion_walker::FollowLinks::Roots);
	choose_latest(&mut selected, last_index(matches, "recursive"), fusion_walker::FollowLinks::Roots);
	choose_latest(
		&mut selected,
		last_index(matches, "dereference_recursive"),
		fusion_walker::FollowLinks::Always,
	);
	if cli.directories == Some(DirectoryAction::Recurse) {
		choose_latest(
			&mut selected,
			last_index(matches, "directories"),
			fusion_walker::FollowLinks::Roots,
		);
	}
	choose_latest(
		&mut selected,
	...
	choose_latest(&mut selected, last_index(matches, "no_follow"), fusion_walker::FollowLinks::Never);
	choose_latest(&mut selected, last_index(matches, "follow_all"), fusion_walker::FollowLinks::Always);
	selected.1
}
...
fn grep_walk_request(root: &Path, follow_links: fusion_walker::FollowLinks) -> fusion_walker::WalkRequest {
	fusion_walker::WalkRequest::new(root)
		...
		.order(fusion_walker::WalkOrder::Unordered)
		.emit_root(true)
		.depth(0, usize::MAX)
		...
		.cache(false)
		.filter(fusion_walker::WalkFilter::all())
}
...
fn search_dir<M: Matcher, W: Write>(
	...
	opts: &Options,
	rules: &PathRules,
	follow_links: fusion_walker::FollowLinks,
	out: &mut W,
	had_error: &mut bool,
) -> io::Result<bool> {
	...
	let walk = request.for_each_entry_with_heartbeat(
		...
		|entry: fusion_walker::EntryMeta<'_>| {
			if opts.quiet && any {
				return Ok(fusion_walker::WalkDecision::Stop);
			}
			if entry.file_type == fusion_walker::FileType::Dir {
			...
			if entry.file_type != fusion_walker::FileType::File
		...
		|error: fusion_walker::DirectoryError<'_>| {
	...
	*had_error |= had_error_state.get();
	match walk {
		Ok(fusion_walker::WalkStatus::Complete | fusion_walker::WalkStatus::Stopped) => Ok(any),
		// Propagate BrokenPipe to stop the walk; the host maps its status.
		Err(fusion_walker::WalkError::Interrupted(error))
			if error.kind() == io::ErrorKind::BrokenPipe =>
		{
			Err(error)
		},
		Err(fusion_walker::WalkError::Interrupted(_)) if host.is_cancelled() => {
		...
		Err(fusion_walker::WalkError::InvalidData { path, message }) => {
			*had_error = true;
			if !opts.no_messages {
				let display_path = display_path_for_operand(operand, resolved, &path);
				let _ = writeln!(
