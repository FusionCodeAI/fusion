...
#[allow(
    dead_code,
    reason = "each variant is used by a different utility feature"
)]
...
mod tests {
    ...
    fn escaped_brace_quantifier_with_no_operand_is_rejected() {
        ...
        assert_eq!(
            ere_err(r"\{1,4\}"),
            Some(BreError::RepetitionOperandMissing)
        );
        assert_eq!(
            ere_err(r"\(\{2\}\)"),
            Some(BreError::RepetitionOperandMissing)
    ...
    fn ere_rejects_repetition_with_no_operand() {
        for pattern in ["^+", "^*", "^?", "^{2}", "*x", "+", "(+)", "a|+b", "(|+)"] {
            assert!(
                ere_repetition_operand_missing(pattern),
