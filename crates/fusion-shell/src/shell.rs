...
use bytes::Bytes;
use flume::Sender;
use fusion_builtins::{BuiltinSet, default_builtins};
#[cfg(not(unix))]
...
async fn create_session_for_run(
    ...
    // Process inspection and control (see `fusion_builtins::process_builtins`).
    ...
    // replaced brush's.
    for (name, registration) in fusion_builtins::process_builtins() {
    ...
    // `fusion_builtins::utility_builtins`): consistent, cross-platform implementations
    ...
    if !uutils_env_disabled(config, "PI_DISABLE_UUTILS_BUILTINS") {
        ...
        for (name, registration) in fusion_builtins::utility_builtins() {
...
mod tests {
    ...
    async fn wait_for_process_name(pid: i32, expected: &str) {
        time::timeout(Duration::from_secs(2), async {
            loop {
                if fusion_builtins::ProcInfo::all().into_iter().any(|process| {
                    process.pid() == pid
                        && process.match_name() == expected
                        && process.status() == fusion_builtins::ProcessStatus::Running
                }) {
                    return;
                }
    ...
    async fn builtins_upstream_of_closed_pipe_are_silent_and_exit_141() {
        ...
        for case in cases {
            let command = format!(
                "seq 1 200000 > '{file}'; set -o pipefail; {case}; echo rc=${{PIPESTATUS[0]}}"
            );
            let (result, output) = execute_captured(command).await;
    ...
    async fn command_v_iterates_all_operands() {
        ...
        let exec = session
            .shell
            .run_string(
                "command -v true false pwd nope-xyz-10544 > out.txt",
                &source_info,
                &params,
            )
            .await
        ...
        assert_eq!(
            exit_code(&exec),
            0,
            "command -v exit code with a resolvable name"
        );
        ...
        let exec = session
            .shell
            .run_string(
                "command -v nope-a-10544 nope-b-10544",
                &source_info,
                &params,
            )
            .await
        ...
        assert_eq!(
            exit_code(&exec),
            1,
            "command -v exit code when nothing resolves"
        );
    ...
    ///
    /// The registry (`fusion_builtins::utility_builtins`) maps a name to a
    ...
    async fn every_process_builtin_is_installed_and_dispatches() {
        ...
        let mut registered: Vec<&'static str> =
            fusion_builtins::process_builtins::<brush_core::extensions::DefaultShellExtensions>()
                .into_iter()
                .map(|(name, _)| name)
                .collect();
    ...
    async fn uutils_head_help_and_bad_flag_do_not_exit_process() {
        ...
        assert_ne!(exit_code(&bad), 0, "invalid flag should be a usage error");
        assert_ne!(
            std::fs::read_to_string(tmp.join("err.txt")).expect("err.txt"),
            ""
        );
