...
const pkgName = '@fusioncode/cli';
const IS_WINDOWS = process.platform === 'win32';
const EXE = IS_WINDOWS ? '.exe' : '';
const BIN_NAME = `fusion${EXE}`;
...
function resolvePlatformPackageDir() {
    const platformPkg = `@fusioncode/cli-${process.platform}-${process.arch}`;
    try {
...
function bootstrapCanonical(sourceBinPath, version) {
    try {
        ...
        const versionedName = `fusion-${version}${EXE}`;
...
function resolveBinary() {
    ...
    if (!platformDir) {
        ...
        console.error(`  Expected sibling package @fusioncode/cli-${process.platform}-${process.arch}.`);
...
const childEnv = { ...process.env, FUSION_MANAGED_BY_NPM: '1' };
