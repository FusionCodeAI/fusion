...
use fusion_edit::modes::hashline::{
    ...
    prefixes::{
        hashline_parse_text, is_read_metadata_line, strip_hashline_prefixes,
        strip_new_line_prefixes, strip_one_leading_hashline_prefix,
    },
...
fn formats_hashline_v4_rows_and_headers() {
    assert_eq!(
        format_hashline_header("src/a.ts", "1A2B"),
        "[src/a.ts#1A2B]"
    );
...
fn input_recovers_apply_patch_header_noise_and_spaces() {
    let patch = Patch::parse(
        "[*** Update File: dir with spaces/a.ts#1A2B]\nPUT 1:\n+x",
        &options(),
    )
    .unwrap();
...
fn input_recovers_headers_nested_in_apply_patch_envelope_markers() {
    ...
    let patch = Patch::parse(
        "[*** Begin Patch] [migrations.ts#5275]\nCUT 33.=38",
        &options(),
    )
    .unwrap();
    assert_eq!(patch.sections.len(), 1);
...
fn prefix_helpers_strip_read_and_diff_shapes() {
    assert_eq!(
        strip_one_leading_hashline_prefix(" >>> + 42:hello"),
        "hello"
    );
    let lines = vec!["[a.ts#1A2B]".into(), "1:one".into(), "2:two".into()];
    assert_eq!(strip_hashline_prefixes(&lines), ["one", "two"]);
    assert_eq!(
        strip_new_line_prefixes(&["+one".into(), "+two".into()]),
        ["one", "two"]
    );
    assert_eq!(
        hashline_parse_text(Some("1:one\r\n2:two\n")),
        ["one", "two"]
    );
