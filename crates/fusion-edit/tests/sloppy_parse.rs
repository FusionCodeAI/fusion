...
fn extracts_an_inline_payload_region_without_swallowing_surrounding_prose() {
    ...
    let excised =
        String::from_utf16(&[&utf16[..regions[0].start], &utf16[regions[0].end..]].concat())
            .unwrap();
    assert_eq!(
        excised,
        "I'll fix the constant now.\n\n\nThat updates the default."
    );
}
...
fn extracts_disjoint_inline_regions_with_narration_between_them() {
    ...
    let regions =
        extract_inline_sloppy_regions(&format!("{first}\nNow the second file:\n{second}"));
    assert_eq!(
        regions
            .iter()
...
fn reports_utf16_offsets_for_inline_regions_after_non_bmp_text() {
    ...
    assert_eq!(
        region.end,
        prefix.encode_utf16().count() + payload.encode_utf16().count() + 1
    );
...
fn envelope_noise_stripping_discards_commentary_until_the_next_opener() {
    ...
    assert_eq!(
        strip_envelope_noise(lines.into_iter().collect()),
        [
            "<SM:EDIT path=\"a.ts\">",
            "<SM:FIND>x</SM:FIND>",
            "<SM:EDIT path=\"b.ts\">",
...
fn hands_back_a_fill_in_skeleton_for_a_truncated_register_rewrite_without_echoing_the_broken_payload()
 {
    ...
    assert!(
        error.contains(
            "»1 after <SM:FIND> reads as the <SM:PUT> separator, leaving <SM:PUT> empty."
        )
    );
    assert!(error.contains(concat!(
        "<SM:EDIT all>\n<SM:FIND>\nenwlineIndex\n",
...
fn rejects_malformed_marker_envelopes_at_parse_time() {
    ...
    assert_eq!(
        error,
        "Operation 1 has an unclosed selection marker ⟪; add closing ⟫."
    );
    let error = message("«\nconst ⟪value│next⟫⟫\n»\nnext", "const value = 1;\n");
    assert_eq!(
        error,
        "Operation 1 has an unmatched closing selection marker ⟫; add opening ⟪."
    );
}
...
fn does_not_split_a_register_reference_glued_to_extra_content() {
    ...
    assert_eq!(
        error,
        "Invalid control line \"»2 extra\"; use only «, «*, », or »N in REWRITE."
    );
}
...
fn rejects_self_forward_and_match_register_references() {
    ...
    assert_eq!(
        self_reference,
        "»1 must reference an earlier operation, not self/forward."
    );
    let forward = message(
    ...
    assert_eq!(
        forward,
        "»2 must reference an earlier operation, not self/forward."
    );
