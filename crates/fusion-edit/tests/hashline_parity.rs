...
fn check_apply(source: &str, case_name: &str, call: &Value) {
    ...
    match (result, call["error"].as_str()) {
        ...
        (Ok(actual), Some(expected)) => {
            panic!(
                "{source}: {case_name}: expected apply error {expected:?}, got {:?}",
                actual.text
            )
        }
    }
...
fn legacy_parser_and_applier_cases_match_byte_for_byte() {
    ...
    assert!(
        calls > covered_names,
        "parity fixtures must exercise calls, not only list names"
    );
}
...
fn snapshot_store_matches_snapshot_contract_cases() {
    ...
    assert_eq!(
        second, first,
        "fuses repeated reads of identical content onto one tag"
    );
    ...
    assert_ne!(
        tag1, tag2,
        "mints a new tag when content changes and retains the prior version"
    );
    ...
    assert!(
        wide.by_hash(&other, &early).is_none(),
        "rejects cross-path lookups"
    );
    ...
    assert_eq!(
        collisions.record(&path, collide_b, Some(&[2])),
        collision_tag
    );
    assert_eq!(
        &*collisions
...
fn pure_format_input_and_streaming_contracts_cover_uncaptured_cases() {
    ...
    for input in [
        ...
        assert!(
            Patch::parse(input, &SplitOptions::default())
                .expect_err(
                    "rejects trailing junk after a snapshot tag even with apply_patch noise"
                )
                .to_string()
                .contains("Input header must be")
        );
...
async fn coding_agent_hashline_executor_cases_run_through_session() {
    ...
    let args = json!({ "input": format!("[a.ts#{a_tag}]\nPUT 1.=1:\n+AAA\n[b.ts#FFFF]\nPUT 1.=1:\n+BBB") });
    assert!(
        atomic
            .apply_json(&args, &common::DiskWriter::default())
            .await
...
async fn seen_line_guard_rejects_hidden_anchors_and_accepts_seen_anchors() {
    ...
    let args = json!({ "input": format!("[a.txt#{tag}]\nPUT 2.=2:\n+TWO") });
    seen.apply_json(&args, &common::DiskWriter::default())
        .await
...
fn preview_for(
    ...
    input: String,
    finished: bool,
) -> fusion_edit::session::PreviewBatch {
...
fn hashline_streaming_preview_cases_preserve_partial_and_final_contracts() {
    ...
    assert_eq!(
        ...
        vec!["a.ts"],
        "{}",
        scenarios["empty-digest"]
    );
    assert!(
        inspected.entries.is_empty(),
        "{}",
        scenarios["empty-digest"]
    );
    ...
    assert_eq!(
        ...
        1,
        "{}",
        scenarios["path-recovery-stream"]
    );
    assert!(batch.files[0].error.is_none());
    assert!(
        batch.files[0]
