...
fn resolves_parent_targets_outside_cwd() {
    ...
    assert_eq!(
        policy.resolve("../outside.txt").unwrap().absolute,
...
fn preserves_absolute_targets_outside_cwd() {
    ...
    assert_eq!(
        policy.resolve(absolute.to_str().unwrap()).unwrap().absolute,
        absolute
    );
}
...
fn canonical_snapshot_keys_fuse_symlink_equivalent_paths() {
    ...
    assert_eq!(
        store.by_hash(&alias_key, &hash).unwrap().text.as_ref(),
        "x\n"
    );
}
