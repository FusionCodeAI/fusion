...
use fusion_edit::{ApplyRequest, EditMode, session::PreviewBatch};
...
async fn streamed_sloppy_previews_grow_and_match_apply() {
    ...
    assert!(
        streamed.contains("+2|    let x = 2;"),
        "streamed diff shows the addition: {streamed}"
    );
    ...
    assert_eq!(
        ws.read("a.rs").unwrap(),
        SOURCE.replace("let x = 1;", "let x = 2;")
    );
}
...
async fn multi_file_failure_stages_nothing() {
    ...
    assert!(
        message.ends_with("No files were modified — sections apply atomically."),
        "{message}"
    );
...
async fn apply_rereads_files_changed_after_preview() {
    ...
    let outcome = session
        ...
        .expect("apply");
    assert!(
        outcome.files[0].diff.contains("+3|BETA"),
        "{}",
        outcome.files[0].diff
    );
...
async fn result_text_uses_compact_preview_and_header() {
    ...
    assert!(
        outcome.files[0].diff.contains("+2|TWO"),
        "{}",
        outcome.files[0].diff
    );
    assert_eq!(
        outcome.files[0].old_text.as_deref(),
        Some("one\ntwo\nthree\n")
    );
    assert_eq!(
        outcome.files[0].new_text.as_deref(),
        Some("one\nTWO\nthree\n")
    );
}
