...
fn seek_sequence_parity() {
    ...
    assert_eq!(
        rstrip.index,
        Some(0),
        "rstrip match ignores trailing whitespace"
    );
    let trim = seek_sequence(&["    foo   ", "   bar\t"], &["foo", "bar"], 0, false, true);
    assert_eq!(
        trim.index,
        Some(0),
        "trim match ignores leading and trailing whitespace"
    );
    let longer = seek_sequence(
        &["just one line"],
        &["too", "many", "lines"],
        0,
        false,
    ...
    assert_eq!(
        longer.index, None,
        "pattern longer than input returns undefined"
    );
    assert_eq!(
    ...
    assert_eq!(
        fuzzy.index,
        Some(0),
        "fuzzy match finds sequence with minor differences"
    );
    assert!(
...
fn parse_diff_hunks_parity() {
    ...
    assert_eq!(
        simple[0].change_context.as_deref(),
        Some("def f():"),
        "parses simple hunk"
    );
...
fn production_apply_patch_parser_parity() {
    ...
    assert_eq!(
        direct[0].diff.as_deref(),
        Some("hi\n"),
        "returns PatchInput[] shape directly"
    );
    let rename = parse_apply_patch(&wrap(
        "*** Update File: a.py\n*** Move to: b.py\n@@\n-old\n+new",
    ))
    ...
    assert!(
        parse_apply_patch(&mismatched).is_err(),
        "mismatched heredoc quotes are not stripped"
    );
    assert!(
...
fn streaming_matcher_parity() {
    let patch = PatchEngine {
        allow_fuzzy: true,
        fuzzy_threshold: 0.95,
    };
    let no_path = patch.inspect(&ArgSnapshot::default());
    assert!(
        no_path.paths.is_empty(),
        "returns undefined when no path is present"
    );
    ...
    assert_eq!(
        ...
        [
            ("src/a.ts".into(), "const a = 1;".into()),
            ("README.md".into(), "# Heading".into())
        ],
...
fn partial_json_edit_entries_parity() {
    let complete = fusion_edit::stream_json::snapshot_from_text(
    ...
    let trailing_open = fusion_edit::stream_json::snapshot_from_text(
        r#"{"path":"a","edits":[{"op":"delete"},{"op":"delete""#,
        false,
        false,
    );
    ...
    let newly_opened = fusion_edit::stream_json::snapshot_from_text(
    ...
    let empty = fusion_edit::stream_json::snapshot_from_text(r#"{"edits":["#, false, false);
    assert!(empty.edits.is_empty(), "leaves empty edits alone");

    let mut absent = ArgStream::new(false);
    absent.push("{}");
    assert!(
        absent.snapshot().edits.is_empty(),
        "keeps all entries when partialJson is undefined"
    );
}
...
fn find_context_line_parity() {
    ...
    assert_eq!(
        whitespace.index,
        Some(0),
        "finds context line with whitespace differences"
    );
    assert!(
        whitespace.confidence > 0.9,
        "finds context line with whitespace differences"
    );
    assert_eq!(
        find_context_line(
    ...
    assert!(
        prefix.confidence > 0.9,
        "finds context line as prefix match"
    );
    ...
    assert_eq!(
        substring.index,
        Some(0),
        "finds context line as substring match"
    );
    assert!(
        substring.confidence > 0.9,
        "finds context line as substring match"
    );
    ...
    assert_eq!(
        fuzzy.index,
        Some(0),
        "falls back to fuzzy match for similar lines"
    );
    assert!(
...
async fn oversized_patch_outcome_prunes_snapshots_but_keeps_diff() {
    ...
    assert!(
        file.snapshots_pruned,
        "prunes oldText / newText while keeping diff and path"
    );
    assert!(
        file.snapshots_pruned,
