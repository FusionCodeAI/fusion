...
use fusion_edit::{EditMode, ModeEngine, modes::replace::ReplaceEngine, stream_json::ArgSnapshot};
use serde_json::json;
...
fn replace_matcher_entries_use_top_level_path_and_new_string() {
    ...
    assert_eq!(
        engine.inspect(&snapshot).entries,
        vec![("src/foo.ts".into(), "x = 1".into())]
    );
}
...
fn preview_uses_compute_edit_diff_error_strings() {
    ...
    assert_eq!(
        empty.preview().files[0].error.as_deref(),
        Some("oldText must not be empty.")
    );
