...
impl Operation {
    fn parse(value: Option<&str>) -> Result<Self, EditError> {
        match value.unwrap_or("update") {
            ...
            other => Err(EditError::apply(format!(
    ...
    if !pattern_mixed && !actual_mixed && pattern_space_only && actual_tab_only {
        ...
        if consistent && !samples.is_empty() {
            let (width, offset) = if samples.len() == 1 {
                ...
                (
                    spaces.checked_div(tabs).filter(|_| spaces % tabs == 0),
                    0_isize,
                )
            } else {
                ...
                if tabs_delta != 0 && spaces_delta > 0 && spaces_delta % tabs_delta == 0 {
                    ...
                    let valid = width > 0
                        && samples.iter().all(|(&tabs, &spaces)| {
                            tabs as isize * width + offset == spaces as isize
                        });
    ...
    let collapse = |lines: &[String]| {
        ...
        while index < lines.len() {
            ...
            while next < lines.len()
                && lines[next] == lines[index]
                && shared.contains(&lines[index])
            {
                next += 1;
            }
            index = next;
...
fn fallback_variants(hunk: &DiffHunk, aggressive: bool) -> Vec<HunkVariant> {
    ...
    let base_old = trimmed.as_ref().map_or(hunk.old_lines.as_slice(), |value| {
        value.old_lines.as_slice()
    });
    let base_new = trimmed.as_ref().map_or(hunk.new_lines.as_slice(), |value| {
        value.new_lines.as_slice()
    });
    ...
    for hunk in hunks {
                    ...
                    && count > 1
                {
                    ...
                    let strategy = result
                        ...
                        .map_or(String::new(), |value| {
                            format!(" Matching strategy: {value}.")
                        });
                    let preview = sequence_previews(
                        lines,
            ...
            && pattern.len() == 1
        {
            ...
            if duplicate_count > 1 {
                ...
                if let Some(found) =
                    find_context_relative(lines, &pattern[0], context, prefer_second)
                {
                    result.index = Some(found);
                }
            }
        ...
        if result.match_count.is_some_and(|count| count > 1) {
            let hint =
                match_hint.or_else(|| line_hint.map(|value| value.saturating_sub(1) as usize));
    ...
    if hunks.len() == 1 {
            ...
            && !hunk.is_end_of_file
        {
            let (mut next, warnings) =
                character_match(content, path, hunk, threshold, allow_fuzzy)?;
...
impl ModeEngine for PatchEngine {
        ...
        for entry in entries {
            ...
            match input.op {
                ...
                Operation::Update => {
                    ...
                    let diff = input.diff.ok_or_else(|| {
                        EditError::apply("Update operation requires diff (hunks)")
                    })?;
                    let hunks = parse_diff_hunks(diff)?;
                    if hunks.is_empty() {
                        return Err(EditError::apply("Diff contains no hunks"));
    ...
    fn inspect(&self, args: &ArgSnapshot) -> Inspection {
        ...
        for entry in &args.edits {
            ...
            {
                file_ops.push(FileOpIntent::Move {
                    from: path.clone(),
                    to: rename.clone(),
                });
            }
...
mod tests {
    ...
    fn trailing_newline_policy_is_preserved() {
        ...
        assert_eq!(
            apply_hunks("one", "a.txt", &[hunk], 0.95, true).unwrap().0,
            "two"
        );
    }
}
