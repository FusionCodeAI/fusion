...
pub fn engine_for(
    ...
    match mode {
        EditMode::Replace => Box::new(replace::ReplaceEngine {
            allow_fuzzy,
            fuzzy_threshold,
        }),
        EditMode::Patch => Box::new(patch::PatchEngine {
            allow_fuzzy,
            fuzzy_threshold,
        }),
        EditMode::ApplyPatch => Box::new(apply_patch::ApplyPatchEngine {
            allow_fuzzy,
            fuzzy_threshold,
        }),
        EditMode::Hashline => Box::new(hashline::HashlineEngine { enforce_seen_lines }),
        EditMode::Sloppy => Box::new(sloppy::SloppyEngine {
            allow_fuzzy,
            fuzzy_threshold,
        }),
    }
