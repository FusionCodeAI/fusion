...
fn fuzzy_occurrences(content: &str, pattern: &str, allow_punctuation: bool) -> Vec<Occurrence> {
    ...
    let offsets = [
        0,
        (pattern.len() - seed_length) / 2,
        pattern.len() - seed_length,
    ];
    ...
    let start_at = |offset| {
        if raw {
            ...
            source_start(
                normalized,
                offset,
                content.map_or(normalized.text.len(), str::len),
            )
        }
    };
    let end_at = |offset| {
        ...
        fn visit(&mut self, position: usize) {
            ...
            if position == self.literal_indices.len() {
                ...
                for (token_index, token) in self.pattern.tokens.iter().enumerate() {
                    ...
                    self.content[capture_start..capture_end]
                        .clone_into(&mut captures[*capture_index]);
                }
                ...
                if let Some(existing) =
                    self.candidates.iter_mut().find(|existing| {
                        existing.start == candidate.start
                            && existing.end == candidate.end
                            && self.pattern.selected_capture_indices.iter().all(|index| {
                                existing.captures[*index] == candidate.captures[*index]
                            })
                    })
                {
            ...
            let gap_tokens = previous_index.map_or(&[][..], |index| {
                &self.pattern.tokens[index + 1..token_index]
            });
            ...
            for occurrence in self.occurrences[&token_index].iter().copied() {
                ...
                if let Some(previous) = previous
                    ...
                    && self.content
                        [self.source_end(previous.end)..self.source_start(occurrence.start)]
                        .contains('\n')
                {
                    continue;
                }
...
fn no_match_error(
    ...
    let (reason, offset) = if occurrences.is_empty() {
        (
            format!(
                "Failed fragment: {} has 0 occurrences.",
                display_fragment(literal)
            ),
            closest.1,
        )
    } else {
        (
            format!(
                "Failed fragment: {} could not align.",
                display_fragment(literal)
            ),
    ...
    let correction = if occurrences.is_empty()
        && closest.2 < 0.35
        && !closest.0.is_empty()
        && standalone
    {
    ...
    if pattern.len() <= 160 {
        for (line, line_offset, normalized, _) in ranked {
                ...
                .chain(std::iter::once(normalized.text.len().saturating_sub(width)))
            {
                ...
                best = (
                    line[raw_start..raw_end].to_owned(),
        ...
        let free = candidates
            ...
            .filter(|candidate| {
                !exclusions.iter().any(|(start, end)| {
                    candidate.match_start < *end && *start < candidate.match_end
                })
            })
            .cloned()
            .collect::<Vec<_>>();
...
fn decode_literal_markers(text: String) -> String {
    text.replace("\0V8LITOPEN\0", SELECT_OPEN)
...
fn punctuation_pair_variants(operation: &Operation) -> Vec<Operation> {
    ...
    while let Some(open_relative) = operation.pattern_text[from..].find(SELECT_OPEN) {
        ...
        let Some(close_relative) = operation.pattern_text[selected_start..].find(SELECT_CLOSE)
        else {
            break;
        };
            ...
            if characters.len() == 2 {
                for length in [2, 1, 3] {
                    for (left, right) in [
                        ...
                        variant.rewrite = OperationRewrite::Inline {
                            replacements: next_replacements,
                        };
...
fn locate_with_recovery(
    ...
    match locate(
        content, &pattern, operation, number, path, exclusions, standalone,
    ) {
        Ok(candidates) => {
        ...
        Err(original) => {
            for candidate in recover_pattern_candidates(
                ...
                if let Ok(candidates) = locate(
                    content, &pattern, &recovered, number, path, exclusions, standalone,
                ) {
            ...
            for mut variant in punctuation_pair_variants(operation) {
                ...
                if let Ok(candidates) = locate(
                    content, &pattern, &variant, number, path, exclusions, standalone,
                ) {
...
fn reconcile_overlap(
    ...
    let project = |edit: &PlannedEdit| {
        format!(
            "{}{}{}",
            &content[start..edit.start],
            edit.replacement,
            &content[edit.end..end]
        )
    };
...
#[allow(
    ...
    reason = "paired index bounds are intentional"
)]
fn apply_operations(
    content: &str,
    input: &str,
        ...
        match &operation.rewrite {
            ...
            OperationRewrite::Explicit { text } => {
                ...
                for located in &candidates {
                    ...
                    {
                        let snapped = snap_line_insertion_offset(
                            content,
                            candidate.start,
                            !base.starts_with('\n'),
                        );
