    ...
    if let Ok(patch) = Patch::parse(
        ...
        for section in patch.sections {
            if let Ok(Some(op)) = section.file_op() {
                match op {
                    FileOp::Rem => file_ops.push(FileOpIntent::Delete {
                        path: section.path.clone(),
                    }),
                    FileOp::Move { dest } => file_ops.push(FileOpIntent::Move {
...
mod tests {
    ...
    fn inspect_extracts_paths_and_added_lines_from_partial_input() {
        let inspected = inspect_input(
            "[*** Update File: src/a.ts#ABCD]\nPUT 1.=1:\n+new\n[src/b.ts]\nCUT 2.=2",
        );
