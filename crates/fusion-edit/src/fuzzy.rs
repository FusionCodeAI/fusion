...
fn run_sequence_passes(
    ...
    let exact = collect_indexed_matches(from, to, |index| {
        matches_at(lines, pattern, index, |a, b| a == b)
    });
    ...
    let trailing = collect_indexed_matches(from, to, |index| {
        matches_at(lines, pattern, index, |a, b| {
            js_trim_end(a) == js_trim_end(b)
        })
    });
    if let Some(result) =
    ...
    let comments = collect_indexed_matches(from, to, |index| {
        matches_at(lines, pattern, index, |a, b| {
            strip_comment_prefix(a) == strip_comment_prefix(b)
        })
    });
    ...
    let prefix = collect_indexed_matches(from, to, |index| {
        matches_at(lines_normalized, pattern_normalized, index, |a, b| {
            norm_starts_with(a, b)
        })
    });
...
pub fn seek_sequence(
    ...
    let lines_normalized: Vec<String> =
        lines.iter().map(|line| normalize_for_fuzzy(line)).collect();
...
pub fn find_context_line(
    ...
    let trimmed = collect_indexed_matches(start_from, end, |index| {
        js_trim(lines[index]) == trimmed_context
    });
    ...
    if context_normalized.chars().count() >= PARTIAL_MATCH_MIN_LENGTH {
        ...
        let all_substrings: Vec<(usize, f64)> = (start_from..lines.len())
            .filter_map(|index| {
                ...
                normalized.contains(&context_normalized).then(|| {
                    (
                        index,
                        context_len as f64 / normalized.chars().count().max(1) as f64,
                    )
                })
            })
            .collect();
...
pub fn replace_text(
        ...
        if exact_count > 0 {
            return Ok(ReplaceResult {
                content: normalized_content
                    .replace(normalized_old.as_ref(), normalized_new.as_ref()),
                count: exact_count,
            });
        }
    ...
    let mut output = String::with_capacity(
        normalized_content.len() - matched.actual_text.len() + adjusted.len(),
    );
...
mod tests {
    ...
    fn sequence_matching_ladder() {
        ...
        assert_eq!(
            seek_sequence(&["one"], &["too", "many"], 0, false, true).index,
            None
        );
    ...
    fn context_matching_ladder() {
        ...
        assert_eq!(
            find_context_line(&["  function foo()  {"], "function foo() {", 0, true, false)
                .strategy,
