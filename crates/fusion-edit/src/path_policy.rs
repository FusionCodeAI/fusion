...
impl PathPolicy {
    ...
    pub fn recover_missing(&self, authored: &str) -> Option<Resolved> {
        ...
        let glob = fusion_walker::CompiledWalkGlob::new([format!("**/{escaped}")]).ok()?;
        let started = Instant::now();
        let request = fusion_walker::WalkRequest::new(&self.cwd)
            .hidden(true)
            .gitignore(true)
            .skip_git(true)
            ...
            .cache(false)
            .limit(2)
            .filter(fusion_walker::WalkFilter::all().glob(glob));
        let result = request
        ...
        Some(Resolved {
            absolute: self.cwd.join(&display),
            display,
        })
    }
    ...
    pub fn enforce_write(
        ...
        if move_to.is_some() {
            return Err(EditError::Plan(
                "Plan mode: renaming files is not allowed.".into(),
            ));
        }
        if op == FileOp::Delete {
            return Err(EditError::Plan(
                "Plan mode: deleting files is not allowed.".into(),
            ));
        }
        if self
            .resolve(display)
...
pub fn is_internal_url(authored: &str) -> bool {
    let normalized = normalize_local_scheme(&expand_path(
        &normalize_local_scheme(authored),
        Path::new(""),
    ));
    INTERNAL_PREFIXES
        .iter()
...
fn percent_decode(value: &str) -> Result<String, String> {
    ...
    while i < bytes.len() {
        if bytes[i] == b'%' {
            ...
            let hex =
                std::str::from_utf8(&bytes[i + 1..i + 3]).map_err(|_| "Invalid URL encoding")?;
...
mod tests {
    ...
    fn unwraps_only_strict_hashline_headers() {
        assert_eq!(
            unwrap_hashline_header_path("[src/a.ts#Ab12]  \n"),
            "src/a.ts"
        );
        assert_eq!(unwrap_hashline_header_path("[src/a.ts]"), "src/a.ts");
        assert_eq!(
            unwrap_hashline_header_path("[src/a.ts#bad]"),
            "[src/a.ts#bad]"
        );
    ...
    fn resolves_plain_expanded_and_internal_paths() {
        ...
        assert_eq!(
            p.resolve("@~/x").unwrap().absolute,
            tmp.path().join("home/x")
        );
        assert_eq!(p.resolve(":./x").unwrap().absolute, tmp.path().join("./x"));
        assert_eq!(
            p.resolve("file:///tmp/a%20b").unwrap().absolute,
    ...
    fn resolves_local_and_vault_roots_and_rejects_escape() {
        ...
        assert_eq!(
            p.resolve("vault://_/a.md").unwrap().absolute,
            tmp.path().join("vault/a.md")
        );
        assert_eq!(
            p.resolve("vault://notes/a.md").unwrap().absolute,
            tmp.path().join("named/a.md")
        );
    }
    ...
    fn recovers_one_suffix_but_not_ambiguous_suffixes() {
        ...
        assert_eq!(
            p.recover_missing("src/a.ts").unwrap().display,
            "deep/src/a.ts"
        );
