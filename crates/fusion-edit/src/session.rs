...
fn recorded_view(file: &StagedFile, written: &str) -> String {
    ...
    if notebook::is_notebook_path(&file.absolute) {
        return notebook::notebook_to_editable_text(written, &file.display).map_or_else(
            |_| file.after.clone(),
            |text| normalize_to_lf(&text).into_owned(),
        );
    }
...
fn format_file_text(file: &StagedFile, header: &str) -> String {
    ...
    let preview =
        build_compact_diff_preview(preview_source, &CompactDiffOptions::default()).preview;
...
pub fn source_parses(code: &str, path: &str) -> bool {
    ...
    fusion_ast::summary::summarize_code(fusion_ast::summary::SummaryOptions {
        code: code.to_owned(),
        lang: None,
...
mod tests {
    ...
    fn trailing_removal_is_trimmed_while_streaming() {
        assert_eq!(
            strip_trailing_unbalanced_removal("+1|a\n-2|b\n-3|c"),
            "+1|a"
        );
    ...
    fn snapshot_pruning_uses_combined_budget() {
        ...
        assert_eq!(
            prune_snapshots(Some(big.clone()), None),
            (Some(big.clone()), None, false)
        );
        assert_eq!(
            prune_snapshots(Some(big), Some("y".into())),
            (None, None, true)
        );
    }
}
