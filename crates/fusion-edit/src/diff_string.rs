...
use fusion_ast::block::{EnclosingBoundaryOptions, LineRange, enclosing_block_boundaries};
use regex::Regex;
...
pub fn generate_diff_string(
    ...
    let parts = fusion_diff::line_changes_str(old, new);
...
pub fn generate_unified_diff_string(
    ...
    let hunks = fusion_diff::structured_patch_hunks_u16(
        &old_utf16,
        &new_utf16,
...
fn parse_compact_diff_line(line: &str) -> Option<ParsedCompactDiffLine<'_>> {
    ...
    Some(ParsedCompactDiffLine {
        kind,
        line_number,
        content: &body[separator + 1..],
    })
}
...
fn parse_one_hunk(
    ...
    if old_start_line.is_some_and(|value| value < 1) {
        return Err(parse_error(
            format!(
                ...
                old_start_line.unwrap_or_default()
            ),
            line_number,
        ));
    }
    if new_start_line.is_some_and(|value| value < 1) {
        return Err(parse_error(
            format!(
                "Line numbers must be >= 1 (got {})",
                new_start_line.unwrap_or_default()
            ),
            line_number,
        ));
    }
    ...
    Ok(ParseHunkResult {
        hunk,
        lines_consumed: parsed_lines + start_index,
    })
}
...
mod tests {
    ...
    fn compact_preview_honors_added_run_options() {
        let diff = (0..8)
            ...
            .collect::<Vec<_>>()
            .join("\n");
        let options = CompactDiffOptions {
            max_added_run_context: Some(1),
            max_unchanged_run: Some(3),
        };
        assert_eq!(
            build_compact_diff_preview(&diff, &options).preview,
            "1:line 1\n…\n8:line 8"
        );
        let alias = CompactDiffOptions {
            max_added_run_context: None,
            max_unchanged_run: Some(1),
        };
        assert_eq!(
            build_compact_diff_preview(&diff, &alias).preview,
            "1:line 1\n…\n8:line 8"
        );
    }
    ...
    fn parses_headers_hints_and_wrappers() {
        ...
        assert_eq!(
            (hint.old_start_line, hint.new_start_line),
            (Some(7), Some(7))
        );
