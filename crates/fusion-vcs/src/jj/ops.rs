    ...
    pub fn working_copy_label(&self) -> Result<Option<String>> {
        self.with_current_repo("jj log", |workspace, repo| {
            Box::pin(async move {
                ...
                Ok(Some(
                    change_id[..prefix_len.min(change_id.len())].to_owned(),
                ))
            })
        })
    }
        ...
        match (before_part.mode, after_part.mode) {
            ...
            (Some(before_mode), Some(after_mode)) => {
                if let Some(operation) = change.copy_operation {
                    output
                        .extend_from_slice(format!("{operation} from {before_path}\n").as_bytes());
...
mod tests {
    ...
    fn portable_jj_reads_work_without_the_jj_cli() {
        ...
        let nul = repo
            ...
            .unwrap();
        assert_eq!(
            parse_status_nul(&nul),
            vec![("A  ".to_owned(), vec!["alpha.txt".to_owned()])]
        );
        assert_eq!(
            repo.changed_files(&crate::DiffOptions::default()).unwrap(),
            vec!["alpha.txt"]
        );
        assert_eq!(
    ...
    fn jj_operations_match_cli() {
        ...
        assert_eq!(
            workspace.working_copy_label().unwrap(),
            parse_label(&label_raw)
        );

        let summary_raw = run_jj(
            temp.path(),
            &["diff", "-r", "@", "--summary", "--ignore-working-copy"],
        );
        assert_eq!(
            workspace.status_summary().unwrap(),
    ...
    fn jj_portable_status_log_and_numstat_match_cli_fixture() {
        ...
        assert!(framed.contains(&(
            "R  ".to_owned(),
            vec!["renamed.txt".to_owned(), "rename.txt".to_owned()]
        )));
