...
impl GitRepo {
    ...
    pub fn current_branch(&self) -> Result<Option<String>> {
        Ok(match self.head()? {
            HeadState::Ref {
                ref_name, branch, ..
            } => Some(branch.unwrap_or(ref_name)),
    ...
    pub fn worktrees(&self) -> Result<Vec<WorktreeEntry>> {
        ...
        let primary_root = self
            ...
            .to_owned();
        let primary_head = parse_head_with_refs(
            &self.info().common_dir.join("HEAD"),
            &self.info().common_dir,
        )?;
    ...
    pub fn log_subjects(&self, count: usize) -> Result<Vec<String>> {
        if self.is_reftable() {
            return cli_lines(
                self.root(),
                &["log", &format!("-n{count}"), "--pretty=format:%s"],
            );
        }
        let repo = self.gix()?;
        let mut out = Vec::new();
    ...
    pub(crate) fn ls_files_at_paths(
        ...
        if self.is_reftable() {
            ...
            return Ok(
                cli_text_owned(self.root(), &args, super::cli::SYNC_TIMEOUT)?
                    .lines()
    ...
    pub fn ls_tree(&self, rev: &str, paths: &[String]) -> Result<Vec<String>> {
        if self.is_reftable() {
            ...
            return Ok(
                cli_text_owned(self.root(), &owned, super::cli::SYNC_TIMEOUT)?
                    .split('\0')
                    .filter(|s| !s.is_empty())
                    .map(str::to_owned)
                    .collect(),
    ...
    pub fn show_blob(&self, spec: &str, max_bytes: Option<usize>) -> Result<ShowResult> {
        ...
        Ok(cap_bytes(
            {
                let mut blob = object.try_into_blob().map_err(|_| Error::ObjectNotFound {
                    spec: spec.to_owned(),
                })?;
                blob.take_data()
            },
...
fn parse_commit_details(raw: &str) -> CommitDetails {
    ...
    CommitDetails {
        sha,
        parents,
        author: CommitAuthor { name, email, date },
        message,
    }
}
...
mod tests {
    ...
    fn read_objects_logs_and_refs() -> TestResult {
        ...
        assert_eq!(
            repo.log_subjects(2)?,
            vec!["second subject", "first subject"]
        );
        ...
        assert_eq!(
            repo.rev_list_touching("HEAD", "two", 10)?,
            vec![second.clone()]
        );
        ...
        assert_eq!(
            repo.lfs_media_dir()?,
            Some(repo.info().common_dir.join("lfs").join("objects"))
        );
    ...
    fn read_worktrees_and_remote_default_branch() -> TestResult {
        ...
        git(
            dir.path(),
            &["worktree", "add", "-b", "linked-branch", &linked_text],
        )?;
