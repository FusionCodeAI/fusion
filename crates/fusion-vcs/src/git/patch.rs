...
impl GitRepo {
    ...
    pub fn cherry_pick(&self, rev: &str) -> Result<()> {
        ...
        if merged_tree == head_tree.detach() {
            return Err(Error::EmptyCherryPick {
                sha: picked_id.to_string(),
            });
        }
        let author = picked
            .author()
        ...
        let committer = repo
            .committer()
            .ok_or_else(|| {
                Error::backend("git cherry-pick", "committer identity is not configured")
            })?
    ...
    pub fn stash_try_pop(&self, reinstate_index: bool) -> Result<bool> {
        ...
        if parents.len() < 2 {
            return Err(Error::backend(
                "git stash pop",
                "stash commit has fewer than two parents",
            ));
        }
        let base = repo
...
fn parse_patch(text: &str) -> std::result::Result<Vec<FilePatch>, ApplyFailure> {
    ...
    if starts.is_empty() {
        return Err(ApplyFailure::Invalid(
            "patch has no diff --git header".into(),
        ));
    }
...
fn split_diff_paths(paths: &str) -> std::result::Result<(&str, &str), ApplyFailure> {
    if paths.starts_with('"') {
        return Err(ApplyFailure::Invalid(
            "quoted paths in diff headers are unsupported".into(),
        ));
    }
    paths
        .split_once(' ')
...
fn apply_file_bytes(
    ...
    for hunk in &patch.hunks {
        ...
        {
            return Err(ApplyFailure::Context(format!(
                "hunk at line {start} does not apply"
            )));
        }
...
fn decode_binary_block(
    ...
    for line in &block.encoded {
        ...
        if decoded_len > decoded.len() {
            return Err(ApplyFailure::Invalid(
                "binary line length exceeds payload".into(),
            ));
        }
...
fn apply_git_delta(base: &[u8], delta: &[u8]) -> std::result::Result<Vec<u8>, ApplyFailure> {
    ...
    if base_size != base.len() {
        return Err(ApplyFailure::Context(
            "binary delta base size mismatch".into(),
        ));
    }
    ...
    while cursor < delta.len() {
        ...
        if command & 0x80 != 0 {
            ...
            for bit in 0..4 {
                if command & (1 << bit) != 0 {
                    offset |= usize::from(*delta.get(cursor).ok_or_else(|| {
                        ApplyFailure::Invalid("truncated delta copy offset".into())
                    })?) << (8 * bit);
                    cursor += 1;
                }
            }
            for bit in 0..3 {
                if command & (1 << (4 + bit)) != 0 {
                    size |= usize::from(*delta.get(cursor).ok_or_else(|| {
                        ApplyFailure::Invalid("truncated delta copy size".into())
                    })?) << (8 * bit);
                    cursor += 1;
                }
            }
...
fn merge_patch_bytes(
    ...
    let path = patch
        ...
        .unwrap_or("file");
    let base_tree = write_tree_map(
        repo,
        &BTreeMap::from([(path.to_owned(), FileEntry::new(base_blob, mode))]),
    )?;
    let ours_tree = write_tree_map(
        repo,
        &BTreeMap::from([(path.to_owned(), FileEntry::new(ours_blob, mode))]),
    )?;
    ...
    let entry = merged.get(path).ok_or_else(|| Error::Conflict {
        paths: vec![path.to_owned()],
    })?;
...
fn tree_map(repo: &gix::Repository, tree: gix::ObjectId) -> Result<BTreeMap<String, FileEntry>> {
    ...
    for entry in index.entries() {
        ...
        map.insert(
            path.to_str_lossy().into_owned(),
            FileEntry::new(entry.id, entry.mode),
        );
    }
    Ok(map)
}
...
mod tests {
    ...
    fn patch_stage_hunks_stages_intent_to_add_and_preserves_unrelated_promise() {
        ...
        assert!(
            status.contains("A  picked.txt"),
            "picked.txt staged: {status}"
        );
        assert!(status.contains("M  base.txt"), "base.txt staged: {status}");
        assert!(
            status.contains(" A promised.txt"),
            "promised.txt keeps intent-to-add: {status}"
        );
    }
    ...
    fn stage_hunks_commit_split_survives_unadvanced_index_mtime() {
        ...
        repo.commit_create(
            "feat: add new file",
            &crate::types::CommitOptions::default(),
        )
    ...
    fn patch_apply_matches_git_for_text_binary_mode_rename_and_no_eof() {
        ...
        git(
            oracle.path(),
            &["apply", "--reverse", "--binary", "change.patch"],
        );
        ...
        git(
            oracle.path(),
            &["apply", "--cached", "--binary", "change.patch"],
        );
        fs::remove_file(patch_file).expect("remove patch");
        assert_eq!(
            git(ours.path(), &["write-tree"]),
            git(oracle.path(), &["write-tree"])
        );
        assert_eq!(
            git(ours.path(), &["status", "--porcelain"]),
            git(oracle.path(), &["status", "--porcelain"])
    ...
    fn patch_cached_alternate_index_matches_git_and_preserves_real_index() {
        ...
        assert_eq!(
            fs::read(&ours_index).expect("ours index after check"),
            ours_before_check
        );
        repository
    ...
    fn patch_cherry_pick_and_stash_are_fail_clean() {
        ...
        assert_eq!(
            fs::read(temp.path().join("file.txt")).expect("read"),
            b"base\ntopic\n"
        );
        assert!(matches!(
            repository.cherry_pick(&topic),
            Err(Error::EmptyCherryPick { .. })
        ));
        ...
        assert!(matches!(
            repo(conflict.path()).cherry_pick(&other),
            Err(Error::Conflict { .. })
        ));
        ...
        assert_eq!(
            fs::read(conflict.path().join("file.txt")).expect("conflict file"),
            b"current\n"
        );
        ...
        assert_eq!(
            fs::read(temp.path().join("ignored.txt")).expect("ignored remains"),
            b"ignored\n"
        );
        ...
        assert_eq!(
            fs::read(temp.path().join("file.txt")).expect("after file"),
            before_file
        );
        assert_eq!(
            fs::read(temp.path().join(".git/index")).expect("after index"),
            before_index
        );
    ...
    fn patch_stash_pop_preserves_older_stack_entry() {
        ...
        assert_eq!(
            fs::read(temp.path().join("file.txt")).expect("second restored"),
            b"second\n"
        );
    }
}
