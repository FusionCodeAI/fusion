...
impl GitRepo {
    ...
    pub fn diff_no_index(&self, left: &Path, right: &Path, binary: bool) -> Result<String> {
        ...
        let old_path = left_file.as_ref().map_or_else(
            || no_index_label(self.root(), right),
            |_| no_index_label(self.root(), left),
        );
        let new_path = right_file.as_ref().map_or_else(
            || no_index_label(self.root(), left),
            |_| no_index_label(self.root(), right),
        );
...
fn render_change(
    ...
    match prepared.operation {
        ...
        gix::diff::blob::platform::prepare_diff::Operation::ExternalCommand { .. } => {
            Err(Error::backend(
                "git diff",
                "external diff drivers cannot be rendered in-process",
            ))
        }
    }
}
        ...
        if match_size < 4096 {
            ...
            for entry in &buckets[hash as usize & mask] {
                ...
                while length < available
                    && source[entry.offset + length] == target[position + length]
                {
                    length += 1;
                }
...
mod tests {
    ...
    fn diff_matches_git_for_worktree_text() {
        ...
        let expected_numstat = git(dir.path(), &["diff", "--numstat"])
            ...
            .collect::<Vec<_>>();
        assert_eq!(
            repo.numstat(&DiffOptions::default()).expect("numstat"),
            expected_numstat
        );
        fs::write(dir.path().join("intent.txt"), "intent\n").expect("write intent");
        git(dir.path(), &["add", "-N", "intent.txt"]);
        assert_eq!(
            repo.diff_text(&DiffOptions::default())
    ...
    fn status_reads_observe_same_tick_stage_on_reused_handle() {
        ...
        assert_eq!(
            repo.status_porcelain(&crate::types::StatusOptions::default())
                .expect("status"),
            git(dir.path(), &["status", "--porcelain"])
        );
        assert_eq!(
            repo.status_summary().expect("summary"),
            crate::types::StatusSummary {
                staged: 2,
                unstaged: 0,
                untracked: 0,
            }
        ...
        assert_eq!(
            repo.ls_files(true, true).expect("untracked files"),
            Vec::<String>::new()
        );
    ...
    fn no_index_text_binary_executable_and_symlink_match_git() {
        ...
        for path in ["untracked.txt", "untracked.dat", "executable", "link"] {
            assert_eq!(
                repo.diff_no_index(Path::new("/dev/null"), Path::new(path), true)
