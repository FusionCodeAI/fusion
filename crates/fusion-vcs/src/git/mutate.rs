...
fn resolve_commit(repo: &gix::Repository, spec: &str) -> Result<gix::hash::ObjectId> {
    ...
    let object = id.object().map_err(|_| Error::ObjectNotFound {
        spec: spec.to_owned(),
    })?;
    object
        .peel_to_commit()
...
#[allow(
    ...
    reason = "matches non-const unix signature"
)]
...
fn resolve_checkout_target(
    ...
    let branch = if rev.starts_with("refs/heads/") {
        ...
        repo.try_find_reference(&full)
            .map_err(|e| Error::backend("git checkout", e))?
            .map(|_| full)
    };
    Ok((
        resolve_commit(repo, branch.as_deref().unwrap_or(rev))?,
        branch,
    ))
}
...
mod tests {
    ...
    fn mutate_stage_commit_amend_and_empty() {
        ...
        assert!(
            repo.commit_create("empty", &CommitOptions::default())
                .is_err()
        );
    ...
    fn stage_all_skips_nested_gitignore_like_git_add() {
        ...
        fs::write(
            temp.path().join("tests/e2e/.gitignore"),
            "screenshots/\ntest-results/\n",
        )
        .unwrap();
        fs::write(
            temp.path().join("tests/e2e/screenshots/homepage.png"),
            "png",
        )
        .unwrap();
        ...
        assert!(
            !status.contains("screenshots"),
            "nested-ignored screenshot was staged:\n{status}"
        );
        assert!(
    ...
    fn stage_explicit_ignored_file_is_skipped() {
        ...
        repo.stage_files(&["secret".into(), "visible".into()])
            .unwrap();
        let status = git(temp.path(), &["status", "--porcelain"]);
        assert!(
            !status.contains("secret"),
            "explicit ignored path was staged:\n{status}"
        );
        assert!(
            status.contains("visible"),
            "explicit unignored path should be staged:\n{status}"
        );
    }
    ...
    fn stage_dot_pathspec_matches_repository_root() {
        ...
        assert!(
            status.contains("M  a"),
            "tracked root file should be staged:\n{status}"
        );
        assert!(
            status.contains("A  visible"),
            "untracked root file should be staged:\n{status}"
        );
        assert!(
    ...
    fn stage_dot_prefixed_directory_pathspec_is_scoped() {
        ...
        let status = git(temp.path(), &["status", "--porcelain"]);
        assert!(
            status.contains("M  dir/tracked"),
            "tracked child should be staged:\n{status}"
        );
        assert!(
            status.contains("A  dir/new"),
            "untracked child should be staged:\n{status}"
        );
        assert!(
            status.contains("?? outside"),
            "unrequested path should stay untracked:\n{status}"
        );
    }
    ...
    fn stage_all_does_not_add_nfd_duplicate_of_nfc_index_path() {
        ...
        assert!(
            !before.contains('\u{301}'),
            "git add should store NFC, got {before:?}"
        );
        ...
        assert_eq!(
            cafe,
            [nfc],
            "native add created a unicode duplicate: {after:?}"
        );
        ...
        assert_eq!(
            cafe,
            [nfc],
            "explicit NFD add created a unicode duplicate: {after_explicit:?}"
        );
    }
    ...
    fn stage_keeps_distinct_nfc_and_nfd_when_precompose_is_off() {
        ...
        assert_eq!(
            cafe.len(),
            2,
            "NFC and NFD must stay distinct when precompose is off: {files:?}"
        );
    ...
    fn commit_hooks_match_git_commit_behavior() {
        ...
        repo.commit_create("original", &CommitOptions::default())
            .unwrap();
        assert_eq!(
        ...
        repo.commit_create("missing hook is skipped", &CommitOptions::default())
            .unwrap();

        fs::write(temp.path().join("b"), "one more\n").unwrap();
        repo.stage_files(&["b".into()]).unwrap();
        write_hook(&pre_commit, "echo should-not-run >&2\nexit 1", false);
        repo.commit_create("non-executable hook is skipped", &CommitOptions::default())
            .unwrap();
        fs::remove_file(&pre_commit).unwrap();
        repo.commit_create(
            "subject\n\nbody\n\n",
        ...
        .unwrap();
        assert_eq!(
            repo.commit_details("HEAD").unwrap().message,
            "subject\n\nbody\n"
        );
    }
    ...
    fn mutate_checkout_branches_and_resets() {
        ...
        assert_eq!(
            git(temp.path(), &["symbolic-ref", "--short", "HEAD"]),
            "other"
        );
    ...
    fn mutate_clean_and_alternate_index() {
        ...
        repo.commit_create("ignore", &CommitOptions::default())
            .unwrap();
    ...
    fn clean_respects_nested_ignores_and_preserves_submodules() {
        ...
        repo.commit_create("add nested gitignore", &CommitOptions::default())
            .unwrap();
        ...
        fs::write(
            checked_out_sub.join("dirty-untracked.txt"),
            "sub untracked\n",
        )
        .unwrap();
        ...
        assert!(
            !initially_empty.exists(),
            "empty untracked directory must be pruned by -d"
        );

        // Ignored empty directory MUST SURVIVE
        assert!(
            ignored_empty.exists(),
            "ignored empty directory must survive clean"
        );

        // Nested ignored file MUST SURVIVE
        assert!(nested.join("secret.env").exists());
        assert_eq!(
            fs::read_to_string(nested.join("secret.env")).unwrap(),
            "secret\n"
        );
        ...
        assert_eq!(
            fs::read_to_string(temp.path().join("global.env")).unwrap(),
            "global-secret\n"
        );
    ...
    fn clone_first_worktree_reconciles_source_state_to_target_tree() {
        ...
        if result.cloned_with.is_some() {
            assert_eq!(
                fs::read_to_string(linked.join("build/out.txt")).unwrap(),
                "ignored\n"
            );
        } else {
            assert!(!linked.join("build/out.txt").exists());
            assert!(
                result.clone_error.is_some() || fusion_iso::clone_candidates(None).is_empty(),
    ...
    /// on `dir`'s filesystem. `fusion_iso::clone_candidates` is a host-level
    ...
    fn clone_works_on(dir: &Path) -> bool {
        ...
        let works = fusion_iso::clone_candidates(None).into_iter().any(|kind| {
    ...
    fn worktree_add_keep_changes_carries_dirty_state_on_both_paths() {
        for clone in [WorktreeClone::Auto, WorktreeClone::Off] {
            ...
            assert_eq!(
                fs::read_to_string(linked.join("a")).unwrap(),
                "staged then edited\n"
            );
            assert!(!linked.join("b").exists());
            assert_eq!(
                fs::read_to_string(linked.join("new/dir/untracked")).unwrap(),
                "untracked\n"
            );
            assert_eq!(
            ...
            assert!(
                rejected.is_err(),
                "keep_changes must require the source HEAD as target"
            );
            let _ = repo.worktree_remove(&linked, true);
        }
    }
    ...
    fn mutate_worktree_and_detach() {
        ...
        assert_eq!(
            detach_git_dir(&linked, &common).unwrap(),
