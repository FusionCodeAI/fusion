...
impl GitRepo {
    ...
    pub fn require(dir: &Path) -> Result<Self> {
        Self::discover(dir)?.ok_or_else(|| Error::NotARepository {
            path: dir.to_owned(),
        })
    }
...
fn resolve_info(
    ...
    let is_reftable = read_optional(&common_dir.join("config"))
        .is_some_and(|config| config_has_reftable(&config));
    Ok(Some(GitRepoInfo {
...
mod tests {
    ...
    fn gitdir_pointer_parsing() {
        ...
        assert_eq!(
            parse_gitdir_pointer("gitdir:../relative"),
            Some("../relative")
        );
