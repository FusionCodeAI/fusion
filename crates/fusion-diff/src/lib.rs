    ...
    match (deletion, insertion) {
        (Some(del), Some(ins)) => {
            ...
            if let Some(start) = start_keep {
                let common_ws_prefix =
                    longest_common_prefix(&old_ws_prefix, &new_ws_prefix).to_vec();
                changes[start].value =
            ...
            if let Some(end) = end_keep {
                let common_ws_suffix =
                    longest_common_suffix(&old_ws_suffix, &new_ws_suffix).to_vec();
                changes[end].value =
        ...
        (Some(del), None) => match (start_keep, end_keep) {
            (Some(start), Some(end)) => {
                ...
                changes[start].value =
                    replace_suffix(&changes[start].value, &new_ws_full, start_ws);
            }
            (None, Some(end)) => {
...
mod tests {
    ...
    fn lines(old: &str, new: &str) -> Vec<(String, bool, bool)> {
        diff_lines_u16(&u16s(old), &u16s(new))
            ...
            .map(|change| {
                (
                    String::from_utf16(&change.value).unwrap(),
                    change.added,
                    change.removed,
                )
            })
    ...
    fn line_runs_preserve_empty_lines() {
        ...
        assert_eq!(
            shaped,
            vec![(2, false, false), (1, false, true), (1, true, false)]
        );
    }
    ...
    fn unpaired_surrogates_diff_as_distinct_content() {
        ...
        assert_eq!(
            shaped,
            vec![(old.to_vec(), false, true), (new.to_vec(), true, false)]
        );
    }
