...
export function RightPanel({
  ...
  return (
    ...
    >
      ...
      {/* Tab 3: Terminal — REAL native macOS Terminal (not in-app markup) */}
      {currentTab === "terminal" && (
        <div data-testid="right-panel-terminal" className="flex-1 flex flex-col items-center justify-center px-6 text-center space-y-3">
          <TerminalIcon className="w-10 h-10 stroke-1 text-zinc-400" />
          <div>
            <p className="text-sm font-semibold text-zinc-900">Native macOS Terminal</p>
          </div>
          <button
            type="button"
            data-testid="open-native-terminal"
            onClick={async () => {
              try {
                const { invoke } = await import("@tauri-apps/api/core");
                await invoke("open_native_terminal", { cwd: workspaceDir || null });
              } catch (err) {
                console.error("[RightPanel] open_native_terminal failed:", err);
              }
            }}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-zinc-900 hover:bg-zinc-800 text-white text-xs font-medium transition-colors cursor-pointer shadow-sm"
          >
            <TerminalIcon className="w-4 h-4" />
            <span>Open Terminal at {workspaceName || "workspace"}</span>
          </button>
        </div>
      )}
    </aside>
